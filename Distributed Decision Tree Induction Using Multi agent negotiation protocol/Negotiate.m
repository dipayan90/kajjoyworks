...
if(isempty(fieldnames(RuleStack1)))
    if(isempty(fieldnames(RuleStack2))) %DB2 has nothing more to propose
        RuleStack = RuleStack3;
    elseif(isempty(fieldnames(RuleStack3))) %DB3 has nothing more to propose
...
if(isempty(fieldnames(RuleStack2)))
    if(isempty(fieldnames(RuleStack1))) %DB1 has nothing more to propose
        RuleStack = RuleStack3;
    elseif(isempty(fieldnames(RuleStack3))) %DB3 has nothing more to propose
        RuleStack = RuleStack1;
    else
    RuleStack = [RuleStack1,RuleStack3]; %DB1 and DB3 have proposed changes
    end
end
if(isempty(fieldnames(RuleStack3)))
    if(isempty(fieldnames(RuleStack1))) %DB1 has nothing more to propose
        RuleStack = RuleStack2;
    elseif(isempty(fieldnames(RuleStack2))) %DB2 has nothing more to propose
        RuleStack = RuleStack1;
    else
    RuleStack = [RuleStack1,RuleStack2]; %DB1 and DB3 have proposed changes
    end
end
